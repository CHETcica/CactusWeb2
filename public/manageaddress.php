...
<body>
  ...
  if($_POST){
    ...
    // ชุดนี้เอาไว้เช็คว่ามีข้อมูล เเล้วสร้างง###################################################################
    $sqlCheck = "SELECT * FROM `address` WHERE `uid`= '$userid'";
    $stmtCheck = $conn->prepare($sqlCheck);
    $stmtCheck->execute();
    $rowCheck = $stmtCheck->fetch(PDO::FETCH_ASSOC);
    $CheckAddress = $rowCheck['aid'];
    
    if(empty($CheckAddress)){
      $sqlua = "INSERT INTO `address`(`uid`,`province`,`district`,`subdistrict`,`house_no`,`Postal_code`) 
      VALUE($userid,'$province','$district','$subdistrict','$house_no',$Postal_code)";
      $stmtua = $conn->prepare($sqlua);
      $stmtua->execute();
    }

    // ชุดนี้เอาไว้อัพข้อมูล  ###################################################################
  ...
  <section class="container pt-5">
    ...
    <div class="row">
      <div class="col-lg-12 col-12">
        <h1 class="profile-header">Address Details</h1>
